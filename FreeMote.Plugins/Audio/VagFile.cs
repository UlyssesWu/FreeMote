...
{
    public class HeVagChannelStream : IDisposable
    {
        public int AdpcmHistory1 = 0;
        public int AdpcmHistory2 = 0;
        public int AdpcmHistory3 = 0;
        public int AdpcmHistory4 = 0;

        public MemoryStream PCMStream { get; set; }
        public BinaryWriter PCMWriter { get; set; }

        public HeVagChannelStream()
        {
            PCMStream = new MemoryStream();
            PCMWriter = new BinaryWriter(PCMStream);
        }

        public byte[] Data => PCMStream?.ToArray();
    ...
    public class VagFile
    {
        ...
        public List<HeVagChannelStream> Channels { get; set; }

        [Obsolete] internal byte[] PcmData { get; set; }
                ...
                else
                {
                    for (int i = 0; i < 14; i++)
                    {
                        ...
                        for (int ii = 0; ii <= 1; ii++)
                        {
                            ...
                            if (PredictNr < 128)
                            {
                                Coefficent = Hist * HEVAGCoeffTable[PredictNr, 0] + Hist2 * HEVAGCoeffTable[PredictNr, 1] +
                                             Hist3 * HEVAGCoeffTable[PredictNr, 2] + Hist4 * HEVAGCoeffTable[PredictNr, 3];
                            }
                            else
                            {
                                Coefficent = 0;
        ...
        public bool LoadFromStream(Stream stream)
        {
            ...
            Channels = new List<HeVagChannelStream>(ChannelCount);
            for (int i = 0; i < ChannelCount; i++)
            {
                Channels.Add(new HeVagChannelStream());
            }

            if (IsStereo)
            {
                Logger.LogWarn("[WARN] Stereo VAG support is experimental.");
            }

            // Get PCM data
            while (br.BaseStream.Position < fileSize)
            {
                foreach (var channelStream in Channels)
                {
                    int Hist = channelStream.AdpcmHistory1;
                    int Hist2 = channelStream.AdpcmHistory2;
                    int Hist3 = channelStream.AdpcmHistory3;
                    int Hist4 = channelStream.AdpcmHistory4;

                    /* external interleave (fixed size), mono */
                    var bytesPerFrame = 0x10;
                    var samplesPerFrame = (bytesPerFrame - 0x02) * 2; /* always 28 */

                    //var PCMStream = channelStream.PCMStream;
                    var PCMWriter = channelStream.PCMWriter;
                    ...
                    else
                    {
                        for (int i = 0; i < 14; i++)
                        {
                            ...
                            for (int j = 0; j < 2; j++)
                            {
                                var coefficent = Hist * HEVAGCoeffTable[coefIndex, 0] + Hist2 * HEVAGCoeffTable[coefIndex, 1] +
                                                 Hist3 * HEVAGCoeffTable[coefIndex, 2] + Hist4 * HEVAGCoeffTable[coefIndex, 3];
                    ...
                    channelStream.AdpcmHistory1 = Hist;
                    channelStream.AdpcmHistory2 = Hist2;
                    channelStream.AdpcmHistory3 = Hist3;
                    channelStream.AdpcmHistory4 = Hist4;
                }
            }
        ...
        static int[] _nibbleToInt =
            new int[] {0, 1, 2, 3, 4, 5, 6, 7, -8, -7, -6, -5, -4, -3, -2, -1};
        ...
        public MemoryStream ToWave()
        {
            ...
            {
                ChannelCount = 1;
                finalData = Channels[0].Data;
            }
            else
            {
                ChannelCount = 2;
                var lData = Channels[0].Data;
                var rData = Channels[1].Data;
                finalData = new byte[lData.Length * 2];
                for (int i = 0; i < lData.Length; i += 2)
                {
                    finalData[i * 2] = lData[i];
                    finalData[i * 2 + 1] = lData[i + 1];
                    finalData[i * 2 + 2] = rData[i];
                    finalData[i * 2 + 3] = rData[i + 1];
                }
            }
            ...
            bw.WriteUTF8("data");
            bw.Write(fileSize);
            ms.Write(finalData, 0x00, fileSize);
